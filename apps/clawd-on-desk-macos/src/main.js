...
const enableClaudeHookRegistration = process.env.CLAWD_ENABLE_CLAUDE_HOOKS === "1";
...
function normalizeWorkflowEventName(event) {
  return String(event || "")
    .trim()
...
function isSessionEndEvent(event) {
  const normalized = normalizeWorkflowEventName(event);
  return normalized === "session_end" || normalized === "gemini_session_end";
}

function isSubagentStopEvent(event) {
  const normalized = normalizeWorkflowEventName(event);
  return normalized === "subagent_stop" || normalized === "sub_agent_stop";
}

function shouldIgnoreDuplicateSessionEnd(sessionId, event) {
  if (!isSessionEndEvent(event)) return false;

  const now = Date.now();
  for (const [id, ts] of recentSessionEnds) {
    if (now - ts > SESSION_END_DEDUPE_MS) {
      recentSessionEnds.delete(id);
    }
  }

  const lastSeen = recentSessionEnds.get(sessionId);
  recentSessionEnds.set(sessionId, now);
  return typeof lastSeen === "number" && now - lastSeen < SESSION_END_DEDUPE_MS;
}

function updateSession(sessionId, state, event) {
  const normalizedEvent = normalizeWorkflowEventName(event);

  if (isSessionEndEvent(normalizedEvent)) {
    ...
    const existing = sessions.get(sessionId);
    if (existing && existing.state === "juggling" && state === "working" && !isSubagentStopEvent(normalizedEvent)) {
      existing.updatedAt = Date.now();
    } else {
  ...
  if (sessions.size === 0 && isSessionEndEvent(normalizedEvent)) {
    setState("sleeping");
    return;
  }
        ...
        try {
          ...
          if (STATE_SVGS[state]) {
            ...
            if (svg) {
              ...
              if (shouldIgnoreDuplicateSessionEnd(sid, event)) {
                res.writeHead(200);
                res.end("duplicate_ignored");
                return;
              }
...
if (!gotTheLock) {
  ...
  app.whenReady().then(() => {
    ...
    // Gemini is the default target in this working copy.
    // Claude hook registration remains available behind an opt-in env var.
    if (enableClaudeHookRegistration) {
      try {
      ...
      console.log("Clawd: Claude hook auto-registration disabled (Gemini mode)");
    }
  });
